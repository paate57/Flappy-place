// server.js
// Server Node.js per canvas collaborativo 2000x2000 pixel SENZA CHUNKING
// Installare: npm install ws pngjs
...
const canvas = new Uint8Array(CANVAS_SIZE * CANVAS_SIZE); // 4 milioni di pixel
...
function loadCanvasFromPNG() {
  ...
  if (!fs.existsSync(savedPath)) return;
...
wss.on('connection', (ws) => {
  ...
  console.log(`Client connesso: ${userId}`);
  ...
  ws.on('close', () => {
    console.log(`Client disconnesso: ${userId}`);
  });
});
